...
def get_token(retry: int = 3) -> str:
    ...
    for attempt in range(retry):
        try:
            ...
            _token_expires = time.time() + int(data.get("expires_in", 86400)) - 300  # 5분 여유
    ...
    raise Exception("❌ KIS 토큰 발급 최종 실패 - API 키 확인 필요")
...
if __name__ == "__main__":
    print("=" * 50)
    print("📈 KIS 주식 급등 알림 봇 v2 시작")
    print("=" * 50)

    send("🤖 <b>주식 급등 알림 봇 ON (v2)</b>\n\n"
         "✅ 한국투자증권 API 연결\n"
         "📡 실시간 스캔 시작 (1분 주기)\n"
         "🛡 오류 자동 복구 기능 추가\n\n"
         "<b>감지 조건</b>\n"
