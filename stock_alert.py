...
③ 뉴스 테마 연동 분석
④ 눌림목 체크 + 이월
⑤ EARLY_DETECT 종목 자동 저장 → tracker.py 연동
"""
...
EARLY_LOG_FILE        = "early_detect_log.json"   # ← tracker.py가 읽는 파일
...
THEME_MAP = {
    "밸류업":   {"desc":"밸류업 프로그램","sectors":["증권","은행","보험","금융"],
                 "stocks":[("001510","SK증권"),("001290","상상인증권"),("005940","NH투자증권"),("016360","삼성증권"),("006800","미래에셋증권"),("039490","키움증권")]},
    "AI반도체": {"desc":"AI/반도체 테마","sectors":["반도체","AI","HBM"],
                 "stocks":[("000660","SK하이닉스"),("005930","삼성전자"),("042700","한미반도체"),("403870","HPSP"),("357780","솔브레인")]},
    "2차전지":  {"desc":"2차전지/배터리 테마","sectors":["배터리","양극재","전해질"],
                 "stocks":[("086520","에코프로"),("247540","에코프로비엠"),("006400","삼성SDI"),("051910","LG화학"),("373220","LG에너지솔루션")]},
    "바이오":   {"desc":"바이오/제약 테마","sectors":["바이오","임상","FDA","신약"],
                 "stocks":[("207940","삼성바이오로직스"),("068270","셀트리온"),("196170","알테오젠"),("009420","한올바이오파마")]},
    "방산":     {"desc":"방위산업 테마","sectors":["방산","방위","무기"],
                 "stocks":[("012450","한화에어로스페이스"),("047810","한국항공우주"),("064350","현대로템"),("042660","한화오션")]},
...
def save_early_detect(stock: dict):
    """EARLY_DETECT 종목을 파일에 자동 저장 → tracker.py가 읽어감"""
    try:
        data = {}
        try:
...
def get_daily_chart(code: str, start: str, end: str) -> list:
    ...
    try:
        ...
        return sorted([{"date":i.get("stck_bsop_date",""),"open":int(i.get("stck_oprc",0)),
                        ...
                        "close":int(i.get("stck_clpr",0))} for i in items if i.get("stck_bsop_date")],
                      key=lambda x: x["date"])
    except:
        return []
...
def analyze_news_theme() -> list:
    ...
    for theme_key, theme_info in THEME_MAP.items():
        if time.time() - _news_alert_history.get(theme_key, 0) < 14400:
            continue
        matched = [h for h in headlines if theme_key in h]
        if not matched:
            continue
        related = []
        for code, name in theme_info["stocks"]:
            try:
                ...
                if cur:
                    related.append({"code":code,"name":name,
                                    "price":cur["price"],"change_rate":cur["change_rate"]})
                time.sleep(0.2)
            except:
                continue
        if not related:
        ...
        signals.append({
            ...
            "matched": matched[0][:50],
            "not_yet":  [s for s in related if s["change_rate"] < 5.0][:5],
            "already":  [s for s in related if s["change_rate"] >= 5.0][:3],
        })
    return signals

def send_news_theme_alert(signal: dict):
    now       = datetime.now().strftime("%H:%M:%S")
    not_yet   = "".join([f"  • <b>{s['name']}</b> ({s['code']}) {s['change_rate']:+.1f}%\n" for s in signal["not_yet"]])
    already   = "".join([f"  • {s['name']} 이미 {s['change_rate']:+.1f}%\n" for s in signal["already"]])
    send(f"🗞️ <b>[테마 연동 알림]</b>\n🕐 {now}\n\n"
...
def check_early_detection() -> list:
    ...
    for stock in candidates:
        code,change_rate,vol_ratio,price = (stock.get("code",""),stock.get("change_rate",0),
                                            stock.get("volume_ratio",0),stock.get("price",0))
        if not code or price < 500: continue
        if change_rate >= UPPER_LIMIT_THRESHOLD: continue
        if change_rate < EARLY_PRICE_MIN or vol_ratio < EARLY_VOLUME_MIN: continue
        try:
        ...
        signals.append({
            ...
            "reasons":[f"🔍 조기 포착! (상한가 전 선진입 기회)",
                       f"📈 현재 +{change_rate:.1f}% 상승 중",
                       f"💥 거래량 {vol_ratio:.0f}배 폭발",
                       f"📊 매수잔량/매도잔량 {hoga_text}",
                       f"✅ 2분 연속 상승 확인"],
            "detected_at":now,
        })
    return signals
...
def is_market_open() -> bool:
    ...
    return dtime(*map(int,MARKET_OPEN.split(":"))) <= now <= dtime(*map(int,MARKET_CLOSE.split(":")))
...
def analyze(stock: dict) -> dict:
    ...
    if vol_ratio >= VOLUME_SURGE_RATIO*2: score+=30; reasons.append(f"💥 거래량 {vol_ratio:.0f}배 폭발!")
    elif vol_ratio >= VOLUME_SURGE_RATIO: score+=20; reasons.append(f"📊 거래량 {vol_ratio:.0f}배 급증")
    if score >= 25:
        try:
        ...
        try:
            ...
            if PULLBACK_MIN <= pullback <= PULLBACK_MAX:
                ...
                signals.append({
                    ...
                    "reasons":[f"🎯 눌림목 진입 시점{carry_text}",
...
def send_alert(s: dict):
    emoji = {"UPPER_LIMIT":"🚨","NEAR_UPPER":"🔥","STRONG_BUY":"💎","SURGE":"📈",
             "ENTRY_POINT":"🎯","EARLY_DETECT":"🔍"}.get(s["signal_type"],"📊")
...
if __name__ == "__main__":
    print("="*50); print("📈 KIS 주식 급등 알림 봇 v8 시작"); print("="*50)
    load_carry_stocks()
    send("🤖 <b>주식 급등 알림 봇 ON (v8)</b>\n\n"
         "✅ 한국투자증권 API 연결\n📡 실시간 스캔 (1분 주기)\n\n"
         "<b>전체 기능</b>\n"
         "🔍 조기 포착: 상한가 전 선진입\n"
         "💾 EARLY_DETECT 자동 저장 → 결과 추적\n"
         "🗞️ 뉴스 테마 연동 (5분마다)\n"
         "📋 DART 공시 분석 (매일 15:35)\n"
         "🎯 눌림목 장 마감까지 실시간 체크\n"
