...
def clean_expired_cache():
    ...
    cleaned = 0
    for cache in [_sector_cache, _daily_cache, _exec_speed_cache]:
        if not isinstance(cache, dict):
            continue
        expired = [k for k, v in cache.items()
                   if isinstance(v, dict) and current_ts - v.get("ts", 0) > CACHE_TTL_SECONDS]
        for k in expired: del cache[k]; cleaned += 1
        if len(cache) > MAX_CACHE_SIZE:
            sortable = [(k, v) for k, v in cache.items() if isinstance(v, dict)]
...
def get_active_market_type() -> str:
    ...
    if 8 <= h < 15 or (h == 15 and m < 30): return "KRX"
    if (h == 15 and m >= 40) or 16 <= h < 20 or h == 20: return "NXT"
    return "CLOSED"

def get_market_params(market_type: str, slot: str = "") -> dict:
    if market_type == "KRX": return KRX_TIME_PARAMS.get(slot, {})
    elif market_type == "NXT": return NXT_TIME_PARAMS.get(slot, {})
    return {}
...
MAX_TELEGRAM_BACKUPS = 5  # [v41.85] 텔레그램 백업 최대 보관 수
...
def _classify_mid_pullback_setup(*, is_intraday: bool = False, resurge_mode: bool = False,
    ...
    elif abs(float(gap_pct or 0.0)) >= 3.0 and int(pullback_days or 0) <= 4:
...
def _get_mid_pullback_execution_profile(result: dict) -> dict:
    ...
    if float(result.get("volume_ratio", 0) or 0) >= 3.0:
...
def _apply_mid_pullback_execution_grade(result: dict) -> dict:
    ...
    reasons.append(f"🎯 실행 A 조건: {MID_PULLBACK_BUCKET_A_GUIDE.get(bucket, '확증 3개 이상')}")
    ...
    if execution_grade != pattern_grade:
        ...
        reasons.append(f"⚠️ 실행 보정: 패턴 {pattern_grade} → 현재 {execution_grade} ({note_reason})")
    result["reasons"] = reasons
    result["pattern_grade"] = pattern_grade
...
def run_mid_pullback_scan():
    ...
    for code, name, theme_desc in all_candidates:
        ...
        if isinstance(_prev_cooldown, dict):
            ...
            _prev_grade = str(_prev_cooldown.get("pattern_grade") or _prev_cooldown.get("grade", "C") or "C").upper()
        else:
